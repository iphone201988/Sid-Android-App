...
object BindingUtils {
    ...
    fun simulationInSights(textView: TextView, ignore: String?) {
        if (ignore == null) {
            return
        }
        textView.text = ignore
    }
...
//        }
    ...
    fun setColorCard(view: CardView, isSelected: String?) {
        if (isSelected != null) {
            view.setCardBackgroundColor(isSelected.toColorInt())
        }
    }

    @BindingAdapter("isSelected","colors", requireAll = true)
    @JvmStatic
    fun setChooseColorCard(view: CardView, isSelected: Boolean?,colors:String?) {
        if (isSelected==true){
            view.setCardBackgroundColor(ContextCompat.getColor(view.context, R.color.text_color_splash))
        }
        else{
            if (colors != null) {
                view.setCardBackgroundColor(colors.toColorInt())
            }
        }
    ...
    @JvmStatic
    fun setColorCardSub(view: GradientView, isSelected: Boolean) {
...
//    }
    ...
    fun rvRecyclerviewSuggest(view: RecyclerView, isSelected: StepperPageModel) {
        ...
        isSelected.tagData?.forEachIndexed { index, tagData ->
        ...
        adapter = SimpleRecyclerViewAdapter(
            ...
            when (v.id) {
                R.id.mainLayout -> {
                    ...
                    val currentText = isSelected.textLastText.get()?.trim() ?: ""

                    if (!m.iselected) {
                        val updatedText = if (currentText.isEmpty()) {
                            itemListData[pos].titleValue
                        } else {
                            "$currentText ${itemListData[pos].titleValue}"
                        }
                        isSelected.textLastText.set(updatedText)
                    } else {
                        val updatedText = currentText.split(" ")                // split by spaces
                            .filter { it != itemListData[pos].titleValue } // remove deselected word
                            .joinToString(" ")         // join again
                        isSelected.textLastText.set(updatedText.trim())
                    }
                    ...
                    isSelected.tagData?.get(pos)?.iselected = m.iselected
    ...
    @JvmStatic
    @BindingAdapter("isoDateText")
    fun setIsoDateText(textView: TextView, isoDate: String?) {
        isoDate?.let {
            try {
...
//        )
    ...
    fun rvInsights3(
        ...
        for (i in isSelected.simulations.indices) {
            val colorIndex = i % colors.size
            val title = if (isSelected.simulations[i].scenarioTitle == "None") {
                isSelected.simulations[i].customMomentText
            } else {
...
//    }
    ...
    @BindingAdapter("setAnsText")
    @JvmStatic
    fun setAnsText(text: TextView, data: EmotionalResponse?) {
        if (data == null) {
            text.text = ""
            return
        }
        ...
        when (data.questionType) {
            "mcq" -> {
                ...
                val index = data.responseValue
                if (!options.isNullOrEmpty() && index in options.indices) {
                    text.text = options[index!!]
                } else {
                    text.text = "" // fallback if index is invalid
                }
            }
            ...
            "text" -> {
                ...
                text.text = "" // fallback for unsupported types
            }
        }
    }
    ...
    fun showOrHidePassword(editText: AppCompatEditText, imageView: ImageView) {
        isPasswordVisible = !isPasswordVisible
        if (isPasswordVisible) {
            // Show the password
            editText.transformationMethod = HideReturnsTransformationMethod.getInstance()
            imageView.setImageResource(R.drawable.unhide_password)
        } else {
            // Hide the password

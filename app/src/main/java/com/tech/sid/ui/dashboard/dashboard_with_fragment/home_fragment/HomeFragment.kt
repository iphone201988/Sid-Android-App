...
class HomeFragment : BaseFragment<FragmentHomeBinding>() {
    ...
    private fun logoutDelete(isLogout: Boolean) {
        ...
        logOutDelete = BaseCustomDialog(
            R.style.Dialog2, requireActivity(), R.layout.lock_mood
        ) { view ->
            view?.let {
                when (it.id) {
                    R.id.yesButton -> {
        ...
        viewModel.onClick.observe(viewLifecycleOwner) {
            when (it?.id) {
                ...
                R.id.bellNotification -> {
                    ...
                    launcher.launch(Intent(requireActivity(), NotificationActivity::class.java))
...
//                    setMood("Overwhelmed")
                ...
                R.id.resume_conversation -> {
                    ChatActivity.scenarioId = simulationId
                                ...
                                if (signUpModel?.success == true) {
                                    ...
                                    if (signUpModel.data?.moodTrend != null) {
                                        Handler(Looper.getMainLooper()).post {
                                            ...
                                            val paddedData = List(7 - rawData.size) { 0 } + rawData
                                            val maxY =
                        ...
                        Constants.HOME_ACCOUNT -> {
                            try {
                                val signUpModel: HomeModel? =
                                    BindingUtils.parseJson(it.data.toString())
                                if (signUpModel?.success == true) {
                                    todayMoodData = signUpModel.todayMood
                                    myMood = todayMoodData
                                    simulationId =
                                        signUpModel.lastSimulation?.simulationId.toString()
                                    ...
                                    if (signUpModel.lastSimulation != null) {
                                        if (signUpModel.lastSimulation.momentTitle?.contains("None of these") == true) {
                                            binding.boundaryWithAFriend.text ="Custom Simulation"
                                        } else {
    ...
    private val launcher =
        registerForActivityResult(ActivityResultContracts.StartActivityForResult()) { result ->
            if (result.resultCode == Activity.RESULT_OK) {
                val isMoodUpdated =
                    result.data?.getBooleanExtra("isMoodUpdated", false) ?: false
                if (isMoodUpdated) {
                    viewModel.homeDashBoardFunction()
                }
            }

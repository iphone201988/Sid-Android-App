...
class AuthCommonVM @Inject constructor(
    ...
    fun socialLoginApi(data: HashMap<String, Any>) {
        CoroutineScope(Dispatchers.IO).launch {
            observeCommon.postValue(Resource.loading(null))
            try {
                val response = apiHelper.apiForRawBody(data, Constants.SOCIAL_LOGIN_API)

                if (response.isSuccessful && response.body() != null) {
                    observeCommon.postValue(Resource.success(Constants.SOCIAL_LOGIN_API, response.body()))
                } else if (response.code() == Constants.UN_AUTHORISED_CODE || Constants.UN_AUTHORISED_STRING == CommonFunctionClass.jsonMessage(
                        response.errorBody()
                    )
                ) {
    ...
    fun resendOtpFunction(data: HashMap<String, Any>) {
        CoroutineScope(Dispatchers.IO).launch {
            ...
            try {
                ...
                if (response.isSuccessful && response.body() != null) {
                    Log.e("resendOtp", "resendOtpFunction: ${response.body()}", )
                    observeCommon.postValue(
                        Resource.success(

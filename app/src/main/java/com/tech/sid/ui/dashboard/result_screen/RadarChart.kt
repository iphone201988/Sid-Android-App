...
//    private val labelPadding = 5f * resources.displayMetrics.density
...
//        strokeWidth = 2f // No stroke for data
//        isAntiAlias = true
//    }
...
//    }
//
//    fun setChartData(labels: List<String>, values: List<Float>, label: String = "Usage", maxValue: Float) {
...
//        val labelDistanceMultiplier = 1.08f
//        for (i in labels.indices) {
//            val angle = Math.toRadians((i * sliceAngle + rotationOffset).toDouble()).toFloat()
...
//            val labelRadius = radius * labelDistanceMultiplier
...
class RadarChart @JvmOverloads constructor(
    ...
    private var labels: List<String> = listOf(
        "Openness", "Agreeableness", "Gratitude",
        "Conscientiousness", "Neuroticism", "Cognitive Style"
    )
    private var values: List<Float> = listOf(10f, 10f, 10f, 10f, 10f, 10f)
    private var maxValue: Float = 10f
    private val labelPadding = 5f * resources.displayMetrics.density
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        for (i in labels.indices) {
            ...
            val adjustedX = when {
                textX < centerX -> textX - textWidth - 5f
                textX > centerX -> textX + 5f
                else -> textX - textWidth / 2
            }

            val adjustedY = when {
                textY < centerY -> textY - 3f  // slightly lift top labels
                textY > centerY -> textY + textHeight / 2.8f // lower bottom labels a bit
                else -> textY
            }
        ...
        fillPaint.shader = LinearGradient(
            bounds.centerX(), bounds.top, bounds.centerX(), bounds.bottom,
            Color.parseColor("#B3D8CCFF"),
            Color.parseColor("#B3A8FFFF"),
            Shader.TileMode.CLAMP
        )
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        val maxTextWidth = labels.maxOfOrNull { textPaint.measureText(it) } ?: 0f
        val textHeight = textPaint.descent() - textPaint.ascent()
        val extraSpace = maxTextWidth + labelPadding
        val minSize = 100f

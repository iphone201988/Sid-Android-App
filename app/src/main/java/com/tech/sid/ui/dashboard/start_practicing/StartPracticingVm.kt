...
class StartPracticingVm @Inject constructor(
    ...
    fun getAiCoachFunction() {
        CoroutineScope(Dispatchers.IO).launch {
            observeCommon.postValue(Resource.loading(null))
            try {
                val response = apiHelper.apiGetOnlyAuthToken( Constants.GET_EMPATHY_OPTIONS_API)

                if (response.isSuccessful && response.body() != null) {
                    observeCommon.postValue(Resource.success(Constants.GET_EMPATHY_OPTIONS_API, response.body()))

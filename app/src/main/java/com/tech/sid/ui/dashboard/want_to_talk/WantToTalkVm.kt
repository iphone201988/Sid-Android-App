...
class WantToTalkVm @Inject constructor(
    ...
    fun getWantToTalkFunction() {
        CoroutineScope(Dispatchers.IO).launch {
            observeCommon.postValue(Resource.loading(null))
            try {

                val response = apiHelper.apiGetOnlyAuthToken( Constants.GET_RELATION_API)

                if (response.isSuccessful && response.body() != null) {
                    observeCommon.postValue(Resource.success(Constants.GET_RELATION_API, response.body()))
